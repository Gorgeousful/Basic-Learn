...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, valid_lens):
        ...
        queries = transpose_qkv(self.W_q(queries), self.num_heads) #(2,4,100) -> (10,4,20)
        ...
        if valid_lens is not None:
            ...
            valid_lens = torch.repeat_interleave(
                valid_lens, repeats=self.num_heads, dim=0) #(3,3,3,3,3,2,2,2,2,2)
...
class DotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, valid_lens=None):
        ...
        scores = torch.bmm(queries, keys.transpose(1,2)) / math.sqrt(d) #(10,4,6)
        self.attention_weights = d2l.masked_softmax(scores, valid_lens)
        return torch.bmm(self.dropout(self.attention_weights), values) #(10,4,6) * (10,6,20) = (10,4,20)
#@save
